...
char      tcp_buffer[BUF_SIZE] = {0}; //client 1
char      tcp_buffer2[BUF_SIZE] = {0}; //client 2
...
void* tcp_thread(void* arg){
    ...
    for(;;){
        ...
        if(valread2 > 0){
            ...
            //search registry
            char GUID[10];
            sprintf(GUID, "%d", search_registry(tcp_buffer2));
            printf("GUID before sending: %s\n", GUID);
            send(new_socket2, GUID, strlen(GUID), 0 ); //send guid of client with desired file( 0 if file not in registry)
        }


        bzero(tcp_buffer, sizeof(tcp_buffer));     //display message                     //flush buffer
        bzero(tcp_buffer2, sizeof(tcp_buffer2));     //display message 
